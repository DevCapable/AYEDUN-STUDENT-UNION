...
<body style="padding-top: 1px">
 ...
 <!-- NAVBAR ENDS HERE--->
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>NATIONAL PRESIDENT</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#">Home</a></li>
              <li class="breadcrumb-item active">National President</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
                ...
                <table id="example2" class="table table-bordered table-hover">
                  <tr>
                    <td colspan="12" id="header">
                      <h2>All National President Since 1980-Now <i class=" fa fa-clock"></i> ({{now()}})<h2>
                    </td>
                  </tr>
                  <tr>
                  ...
                  @foreach($NationalPresidents as $NationalPresident)
                  <tr>
                    <td width="80px">{{ $NationalPresident->full_name }}</td>
                    <td width="135px">{{ $NationalPresident->compound }}</td>
                    <td width="80px">{{ $NationalPresident->gender }}</td>
                    <td width="100px">{{ $NationalPresident->discepline }}</td>
                    <td width="80px">{{ $NationalPresident->institution }}</td>
                    <td width="100px">{{ $NationalPresident->date_of_birth }}</td>
                    <td width="100px">{{ $NationalPresident->year_of_tenure_from }}</td>
                    <td width="100px">{{ $NationalPresident->year_of_tenure_to }}</td>
                    <td width="100px">{{ $NationalPresident->sport }}</td>
                    <td width="100px">{{ $NationalPresident->history }}</td>
                    <td width="700px"> <img src="{{ asset($NationalPresident->picture) }}" class="img-thumbnail"
                      width="200px" width="200px" style="box-shadow: 0px 2px 7px 2px gray ;"
                      alt="Ayedun Students' Union" style="border-radius: 100px"/></td>
                    <td width="340px">


                      <a onclick="return confirm('Are you sure you want to delete {{$NationalPresident->full_name}}?')"
                        href="{{url('administrator/Delete_National_President', $NationalPresident->id)}}"
                        title="Delete National President">
                        <i class="fas fa-trash text-danger  fa-lg"></i></a>

                      <a title="Click to Like"
                        href="{{ url('administrator/Edit_National_President/'.$NationalPresident->id) }}"
                    ...
                    <div class="col-md-6">
                      <div class="form-group {{ $errors->has('institution') ? 'has-error' : '' }}">
                        ...
                        <input type="text" name="institution" class="form-control" id="institution"
                          value="{{ old('institution') }}" placeholder="Enter your descipline">
  ...
  <script>
    ...
    Toast.fire({
      icon: 'success',
      title: 'Administrator! Here You Can Manage National President'
    })
  </script>
</body>
