        ...
        <div class="row">
            ...
            <div class="col-sm-6">
                ...
                <form method="POST" action="{{ url('user/logout') }}" id="logout" novalidate>
                    @csrf
                    <button style=" float: right;" type="submit" class="btn btn-sm btn-outline-primary"><i class="fas fa-sign-out-alt"></i>Log Out</button>
                </form>
            </div>
        </div>
        <div class="row">
            <div class="col-lg-12 col-md-12">
              ...
              @foreach ($getMessageForUsers as $item1) 
                <table>
                     <tr>
                        ...
                        <td>
                        <p style="border-color: grey; border-style: solid; border-color:green; border-radius: 100px; padding: 30px; width: 100%">
                        {{ $item1->message }}
                        </p>
                        </td>
                        <td> <span class="badge badge-info -">
                            @if( Carbon\Carbon::parse($item1->created_at)<"20" )
                                {{ "just now" }}
             ...
             @endforeach
                        ...
                        <div class="card-footer">
                           @if($getMessageForUsers->links() >="6")
                           <i style="color: red; font-weight: 100%;"> {{ 'Always Click on the last number to see your current Chats' }}</i> 
                           {{ $getMessageForUsers->links() }} 
                          <a onclick="return confirm('are you sure you want to clear all these chats? it has no reverse!')" href="{{ url('user/clearChats') }}"> <button style="float: right"  class="btn btn-danger btn-sm">Clear Chats</button></a>
                           @endif 
                        </div>
                        <div class="card-footer">
