            ...
            <div class="col-lg-12 col-md-12" style="padding-top: 20px;">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title pull-left text-uppercase text-info font-weight-bolder">MISS ASU'S List</h3>
                    ...
                    <div class="card-body" style="overflow-x:auto;">
                        <div class="row el-element-overlay">
                            @foreach ($ViewAllMissAsus as $ViewAllMissAsu)<br />
                            <div class="col-lg-3 col-md-3 col-sm-6" style="padding-top: 20px">
                                <div class="card bg-white card-hover" >
                                    <div class="card-header">
                                        
                                        <span class="badge badge-success pull-right">{{ $ViewAllMissAsu->gender }}</span>
                                        <br>
                                        <span class="badge badge-success pull-left">From ({{ $ViewAllMissAsu->year_of_tenure_from }}) To ({{ $ViewAllMissAsu->year_of_tenure_to }})</span>
                                    </div>
                                    <div class="card-body" style="height: 100%; width: 100%">
                                        <div class="card-title text-center">
                                            <img src="{{ asset($ViewAllMissAsu->picture)}}" class="img_thumbnails" width="200px"
                                                height="200px" class="img-thumbnail" style="box-shadow: 0px 2px 7px 2px gray ;"
                                                alt="Ayedun Students' Union"/><br />
                                            {{ strtoupper($ViewAllMissAsu->full_name) }}
                                        </div>
                                        <br />
                                    </div>
                                    <div class="card-footer">
                                        <a title="Click to comment" href="{{ url('ViewAllMissAsus/like/'.$ViewAllMissAsu->id) }}"
                                            class="btn btn-outline-info btn-sm pull-left">
                                            <i class="fa fa-comments"></i>
                                        </a>
                                        <a href="{{ url('user/ViewMissAsuProfile/'.$ViewAllMissAsu->id) }}"
                                            class="btn btn-outline-primary btn-sm pull-right">
                                            <i class="fa fa-eye"></i> PROFILE
                                        </a>
                                        <span class="badge badge-success pull-right">{{ $ViewAllMissAsu->compound }}</span>
                                    </div>
                                </div>
                            </div>
                            @endforeach
