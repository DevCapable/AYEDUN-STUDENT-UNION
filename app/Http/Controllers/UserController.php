...
class UserController extends Controller
{
    ...
    public function logout(Request $request)
    {
        ...
        $updateAccount = UserAccount::where('id_number', $verifyUser->id_number)->update([
            'is_online' => '0'
        ]);
        if ($updateAccount) {
    ...
    public function startChatting(Request $request, $id)
    {
        ...
        if ($verifyUser) {
            try {
                $getMessageForUsers = Message::where('to_who', $verifyUser->username)->where('from_who', $id)->OrWhere('from_who', $verifyUser->username)->where('to_who', $id)->orderBy('created_at', 'asc')->paginate();
                // fetch current user
