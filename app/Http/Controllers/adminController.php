...
class adminController extends Controller
{
    ...
    public function Manage_Users(Request $request)
    {
        ...
        if ($admin) {
            try {
                ...
                if ($userRequest == '') {
                    $users = UserAccount::orderBy('username', 'ASC')->Paginate(10);
                   
                    //total message unread
                    $unreadMessage = AdminIbox::where('status', 'notRead')->count();
                    //get new notification
                    $AdminInboxs = AdminIbox::where('status', 'notRead')->orderBy('created_at', 'desc')->Paginate(4);
                    $userPost = Post::where('id', '!=', '')->orderBy('username', 'desc')->Paginate(10);
    ...
    public function deleteUserPost(Request $request, $id)
    {
        $loggedInUser = $request->session()->get('admin');
        #get user data here
        $user = AdminAccount::where('AdminName', $loggedInUser)->first();
        if ($user) {
            try {
                $postId = $id;
    ...
    public function SendSignal(Request $request, $id)
    {
        ...
        if ($loggedInAdmin) {
            try {
                ...
                //fetch user from user account
                $poster =UserAccount::where('username',$id)->first();
              
                           // dd($poster);
                //admn sending warning message
                Message::Create([
                    'from_who'=> $loggedInAdmin,
                    'to_who'=>$poster->username,
                    'message'=>'Dear User, Please Mind what You Post on the Platform, this is strictly a warning notice and it attracts suspension after the third wave of warning signals thanks for your co-opperation so far.'
                ]);
                AdminSentMail::Create([
                    'from_who'=> $loggedInAdmin,
                    'email'=>$poster->username,
                    'type_of_issue'=>'Warning Signal',
                    'details'=>'Dear User, Please Mind what You Post on the Platform, this is strictly a warning notice and it attracts suspension after the third wave of warning signals thanks for your co-opperation so far.'
                ]);
                $notification = array(
                    'message'=> 'Warning Signals Sent successfully',
                    'alert-type'=>'success'
                );
                return redirect()->back()->with($notification);
            }catch(\Exception $ex){
                dd($ex);
                $notification = array(
                    'message'=> 'Error occured While sending Warning signal Please try again later',
                    'alert-type'=>'error'
                );
                return redirect()->back()->with($notification);
    ...
    public function ChangeCompoundDp(Request $request)
    {
        ...
        if ($VerifyUser) {
            try {
                ...
                Compound::where('id', $id)->update([
                    'head_image' => 'public/images/' . $imageName
                ]);
    ...
    public function Manage_MissAsu(Request $request)
    {
        ...
        if ($admin) {
            ...
            $unreadMessage = AdminIbox::where('status', 'notRead')->count();
             //get new notification
                $AdminInboxs = AdminIbox::where('status', 'notRead')->orderBy('created_at', 'desc')->paginate(3);
    ...
    public function Inbox(Request $request)
    {
        ...
        if ($admin) {
            try {
                # get compounds data
                $AdminInboxs = AdminIbox::where('status', 'notRead')->orderBy('created_at', 'desc')->Paginate(4);
                $messages = AdminIbox::where('status', 'read')->orWhere('status','notRead')->orderBy('created_at', 'desc')->paginate(5);
                //total message unread
                $unreadMessage = AdminIbox::where('status', 'notRead')->count();
                 $sentMail = adminSentMail::count();
                // total number of message inbox
                $AllMessage = AdminIbox::count();
               
                    $AllSentMessages = AdminSentMail::count();
    ...
    public function sentMail(Request $request)
    {
        ...
        if ($admin) {
            try {
                # get compounds data
                $AdminInboxs = AdminIbox::orderBy('from_who', 'asc')->paginate(3);
                 # get compounds data
                 $AdminSentMaill = AdminSentMail::orderBy('from_who', 'asc')->paginate(10);
                 $sentMail = adminSentMail::count();
                //total message unread
                $unreadMessage = AdminIbox::where('status', 'notRead')->count();
                // total number of message inbox
                $AllMessage = AdminIbox::count();
                $AllSentMessages = AdminSentMail::count();
                return view::make('admin/sentMail')->with([
                    'unreadMessage' => $unreadMessage,
                    'AllMessage' => $AllMessage,
                    'admin' => $admin,
                    'AdminSentMaill'=>$AdminSentMaill,
                    'AdminInboxs' => $AdminInboxs,
                    'sentMail' => $sentMail,
                      'AllSentMessages'=>$AllSentMessages
                ]);
    ...
    public function readSentMail(Request $request, $id)
    {
        // get session data
        $loggedInAdmin = $request->session()->get('admin');
        $admin = AdminAccount::where('AdminName', $loggedInAdmin)->first();
        if ($admin) {
            try {
                // get message from the table
                $Message = AdminSentMail::where('id', $id)->first();
                if ($Message) {
                    //total message unread
                    $unreadMessage = AdminIbox::where('status', 'notRead')->count();
                    //get new notification
                    $AdminInboxs = AdminSentMail::where('status', 'notRead')->orderBy('created_at', 'desc')->paginate(2);
                    // total number of message inbox
                    $AllMessage = AdminIbox::count();
                     $sentMail = adminSentMail::count();
                    $AllSentMessages = AdminSentMail::count();
                    return view::make('admin/readAdminSentMail')->with([
                        'unreadMessage' => $unreadMessage,
                        'AdminInboxs' => $AdminInboxs,
                        'admin' => $admin,
                        'Message' => $Message,
    ...
    public function deleteMessage(Request $request, $id)
    {
        ...
        if ($admin) {
            try {
                ...
                return redirect()->to('administrator/Inbox');
            } catch (\Exception $ex) {
    ...
    public function deleteSentMail(Request $request, $id)
    {
        $loggedInUser = $request->session()->get('admin');
        // get user data
        $admin = AdminAccount::where('AdminName', $loggedInUser)->first();
        if ($admin) {
            try {
                AdminSentMail::where('id', $id)->delete();

                return redirect()->to('administrator/sent-mail');
            } catch (\Exception $ex) {
    ...
    public function deleteAllSentMail(Request $request)
    {
        $loggedInUser = $request->session()->get('admin');
        // get user data
        $admin = AdminAccount::where('AdminName', $loggedInUser)->first();
        if ($admin) {
            try {
              AdminSentMail::all()->delete();
              
                $notification = array(
                    'message' => 'All Sent Mails Deleted Successfully',
                    'alert-type' => 'success'
                );
                return redirect()->back()->with($notification);
            } catch (\Exception $ex) {
    ...
    public function sendMail(Request $request)
    {
        ...
        if ($admin) {
            try {
                ...
                $AllMessage = AdminIbox::count();
                  $sentMail = adminSentMail::count();
               $getUsers = UserAccount::get();
                    $AllSentMessages = AdminSentMail::count();
                return view::make('admin/send-mail')->with([
                    'unreadMessage' => $unreadMessage,
                    'AllMessage' => $AllMessage,
                    'admin' => $admin,
                    'AdminInboxs' => $AdminInboxs,
    ...
    public function sendMailToAllUsers(Request $request)
    {
        ...
        if ($admin) {
            try {
               ...
               $getUsers = UserAccount::get();
                    $AllSentMessages = AdminSentMail::count();
                return view::make('admin/send-mail-to-all-users')->with([
                    'unreadMessage' => $unreadMessage,
                    'AllMessage' => $AllMessage,
                    'admin' => $admin,
                    'AdminInboxs' => $AdminInboxs,
